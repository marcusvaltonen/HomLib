...
class HPatchesPipeline(EvalPipeline):
    ...
    def run_eval(self, loader, pred_file):
        ...
        conf = self.conf.eval
        print(conf)
        num_ransac_iter = 1600

        test_thresholds = (
            ([conf.ransac_th] if conf.ransac_th > 0 else [2.5, 3.0, 3.5, 4.0])
            ...
            for solver in homlib_solvers:
                for th in test_thresholds:
                    pose_results_i = eval_homography_robust(
                        ...
                        "ransac_th": th,
                        "image_size0": data["view0"]["image_size"].detach().cpu().numpy().tolist(),
                        "image_size1": data["view1"]["image_size"].detach().cpu().numpy().tolist(),
                        "solver": solver,
                        "num_ransac_iter": num_ransac_iter
                        },
                    )

                    pose_results_i[f'H_error_{solver}'] = pose_results_i.pop('H_error_ransac')
                    pose_results_i[f'time_{solver}'] = pose_results_i.pop('time')
        ...
        for solver in homlib_solvers:
            best_pose_results, best_th = eval_poses(
                pose_results, auc_ths=auc_ths, key=f"H_error_{solver}", unit="px"
            )
        ...
        # all solvers
        all_errors = {solver: results[f"H_error_{solver}"] for solver in homlib_solvers}
        all_errors["DLT"] = results["H_error_dlt"]
        ...
        for solver in homlib_solvers:
            np.savetxt(f'hpatches_time_mAA_{solver}_{num_ransac_iter:05d}.npy', [summaries[f'mtime_{solver}'], summaries[f'H_error_{solver}_mAA']], delimiter=',')
